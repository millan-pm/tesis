...
yv = np.linspace(1e-6, 10, q_points)                 # size parameter
n1v = np.linspace(1, 4, n_points)                    # índice de la esfera
n2 = 1                  # índice del medio    
wl = 3e-6               # longitud de onda
k = 2*np.pi/wl          # número de onda

z_imp = 376.86          # impedancia del medio
E0 = 1.0                # amplitud del campo
l_max = 10              # orden máximo del desarrollo
nr = 1                  # puntos en r
...
Q_sca_poynting=np.zeros((q_points, n_points))
g = np.zeros((q_points, n_points))
for i, y in enumerate(yv):
    if i % 10 == 0: print(f"Procesando i={i}")
    for j, n1 in enumerate(n1v):
        q = y/n1
        ...
        a = q / k
        Q_sca_poynting[i][j] = C_sca_poynting / (np.pi * a**2)

#%% Plots
plt.figure(figsize=(10,8))
Y, N = np.meshgrid(yv, n1v, indexing='ij')
plt.pcolormesh(Y, N, Q_sca_poynting)
...
plt.figure(figsize=(10,8))
Y, N = np.meshgrid(yv, n1v, indexing='ij')
plt.pcolormesh(Y, N, g, cmap='seismic', norm=colors.CenteredNorm())
plt.xlabel('Size parameter')
plt.ylabel('Refractive index')
plt.colorbar()
plt.xlim(0,10)
...
ni = 3.5
i_n = np.argmin(np.abs(n1v-ni))

plt.plot(3.5*qv, np.pi*Q_sca_poynting[:, i_n])
